	...
	$.getJSON('http://www.htyou.com' + '/mobile/ipad_queryTourLine.action?jsoncallback=?&KeyWords=%E6%95%B0%E4%BA%BA%E5%AE%9A%E5%88%B6', function (result) {
		...
		var OutHTML   = React.createClass({
			...
			render: function () {
				...
				var _tempFloatButtonList = buttonProps.map(function(unit, index){
					return (
						<FloatButton {...unit} key={index}/>
					);
				});
				return (
					<div>
						<h1 onClick={this.clickHandle}>{this.state.test}</h1>
						<div id="button-panel">
							{_tempFloatButtonList}
							<div style={{clear:'both'}}></div>
						</div>
						{_tempList}
					</div>
